...
case $CIRCLE_NODE_INDEX in
 ...
 export KERNONLY=; export EXTRAFLAGS=; export LINUX='-r linux'" >> $HOME/.bashrc
	;;
    1)
	echo "hw i486"
	echo "export PLATFORM=hw; export MACHINE=i486; export ELF=elf; export TESTS=qemu;\
 export KERNONLY=; export EXTRAFLAGS='-- -F ACLFLAGS=-m32 -F ACLFLAGS=-march=i686'; export LINUX='-r linux'" >> $HOME/.bashrc
	;;
    2)
	echo "hw kernonly"
	echo "export PLATFORM=hw; export MACHINE=x86_64; export TESTS=none;\
export KERNONLY=-k; export EXTRAFLAGS=; export LINUX='-r linux'" >> $HOME/.bashrc
	;;
    3)
	echo "xen kernonly"
 ...
 export KERNONLY=; export EXTRAFLAGS='-- -F ACLFLAGS=-m32'; export LINUX='-r linux'" >> $HOME/.bashrc
	;;
esac
